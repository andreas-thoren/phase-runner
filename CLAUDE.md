...
- **`GenericDetails`**: OneToOne → Workout. No extra fields beyond DetailBase (duration, load, additional_data).
- `DetailBase.save()` always calls `full_clean()`, enforcing model validation on every save.
- **GUI fields**: dynamic subtype-specific fields stored in `additional_data["gui_fields"]`. Schema defined on `WorkoutSubtype.gui_schema`. Create/edit views render a dropdown to add fields; detail views display them read-only.
