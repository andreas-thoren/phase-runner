...
- **`SummaryStatsFragmentView`**: GET-only `DetailView` at `plan-<int:macro_pk>/summary/stats/` (name: `summary_stats_fragment`). Renders the partial `workouts/partials/summary_stats.html` containing the aggregate summary-stats table (weekly averages + longest + total distance). Module-level helper `_build_summary_stats(rows, cutoff)` aggregates over microcycles whose `end_date <= cutoff`, where `cutoff = min(date.today(), macro.end_date)`. Always uses only `COMPLETED` workouts (independent of the main table's status filter) so averages reflect actual performance. Triggered by the bar-chart icon-btn in the summary toolbar via `summary_stats_toggle.js` — first click fetches and injects the fragment; subsequent clicks just toggle `hidden`. No persistence across reloads.
