...
- **`MacrocycleSummaryView`**: read-only overview table (extends `DetailView`, not `FormContextMixin`). Hydrates the macrocycle, flattens mesocycles→microcycles into rows with planned goals (distance, long session) on the left and aggregated actual workout stats (sessions, distance, load, cross, strength) on the right. Sport-agnostic: uses the macrocycle's `primary_sport` to determine which workout subtype counts as primary sessions vs cross-training. Column headings are dynamic — e.g., "Runs" / "Long run" for running, "Rides" / "Long ride" for cycling (via `SESSION_LABELS` and `LONG_SESSION_LABELS` dicts in `enums.py`). Single query via `_aggregate_workouts()` buckets workouts into microcycles by date range. Row building and label resolution are in module-level helpers `_build_summary_rows(macro, user)` and `_summary_col_labels(sport)`, shared with `ExportPlanSummaryView`. The "Plan summary" aggregate is loaded lazily via `PlanSummaryFragmentView` on Σ button click, not rendered eagerly here. Template: `macrocycle_summary.html` (extends `base.html`). URL: `plan-<int:macro_pk>/summary/` (name: `macrocycle_summary`). Linked from the macrocycle detail page toolbar when mesocycles exist.
- **`PlanSummaryFragmentView`**: GET-only `DetailView` at `plan-<int:macro_pk>/summary/plan-stats/` (name: `plan_summary_fragment`). Renders the partial `workouts/partials/plan_summary.html` containing the aggregate plan-summary table (weekly averages + longest + total distance). Module-level helper `_build_plan_summary(rows, cutoff)` aggregates over microcycles whose `end_date <= cutoff`, where `cutoff = min(date.today(), macro.end_date)`. Always uses only `COMPLETED` workouts (independent of the main table's status filter) so averages reflect actual performance. Triggered by the Σ icon-btn in the summary toolbar via `plan_summary_toggle.js` — first click fetches and injects the fragment; subsequent clicks just toggle `hidden`. No persistence across reloads.
